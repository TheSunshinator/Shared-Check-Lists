...
/**
 * Fragment that manages a single list
 * <p>
 * Created by The Sunshinator on 18/04/2017.
 */
public class ListFragment
  ...
  private void initRemoveUserDialog() {
    ...
    mvUserList = (RecyclerView) mRemoveUserDialog.findViewById( R.id.list );
  ...
  private void initAddUserDialog() {
    ...
    mvNewUser = (EditText) mAddUserDialog.findViewById( R.id.email_new );
    mvPermAddItems = (CheckBox) mAddUserDialog.findViewById( R.id.perm_add_item );
    mvPermCheck = (CheckBox) mAddUserDialog.findViewById( R.id.perm_check );
    mvPermAddUsers = (CheckBox) mAddUserDialog.findViewById( R.id.perm_users_add );
    mvPermRemoveUsers = (CheckBox) mAddUserDialog.findViewById( R.id.perm_users_remove );
    mvPermClean = (CheckBox) mAddUserDialog.findViewById( R.id.perm_clean );
    mvPermDelete = (CheckBox) mAddUserDialog.findViewById( R.id.perm_delete );
  ...
  private void initNewEntryDialog() {
    ...
    mvNewEntry = (EditText) mNewEntryDialog.findViewById( R.id.entry );
  ...
  private void initViews( View vRoot ) {
    ...
    mvList = (RecyclerView) vRoot.findViewById( R.id.list );
  ...
  private int computeRightsCode() {
    ...
    if ( mvPermClean.isChecked() ) { rightsCode |= Constants.MASK_RIGHT_CLEAN; }
    if ( mvPermAddItems.isChecked() ) { rightsCode |= Constants.MASK_RIGHT_ADD_ITEM; }
    if ( mvPermAddUsers.isChecked() ) { rightsCode |= Constants.MASK_RIGHT_ADD_USERS; }
    if ( mvPermCheck.isChecked() ) { rightsCode |= Constants.MASK_RIGHT_CHECK; }
    if ( mvPermDelete.isChecked() ) { rightsCode |= Constants.MASK_RIGHT_DELETE; }
    if ( mvPermRemoveUsers.isChecked() ) { rightsCode |= Constants.MASK_RIGHT_REMOVE_USERS; }
  ...
  private final ItemClickCallback l_ItemClicked = new ItemClickCallback() {
    ...
    public void onItemClick( DatabaseReference ref, CheckListEntry entry ) {
      ...
      if ( ( mCheckList.getRightsOf( currentUser.getEmail() ) & Constants.MASK_RIGHT_CHECK )
           != Constants.MASK_RIGHT_CHECK ) { return; }
